...
describe('LiquidityBitmapTest', function () {
    ...
    describe('setInRage', async function () {
        ...
        describe("should unset bit in range correctly", async () => {
            it("should unset bit from 255 to 600", async () => {
                await setBitsInRage2(255, 600);
                const result = await liquidityBitmap.unsetBitsRange(255, 600)
                expect(await liquidityBitmap.hasLiquidity(255)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(600)).eq(false)
            })

            it("should unset bit from 255 to 400, should not unset 401", async () => {
                await setBitsInRage2(255, 401);
                const resul = await liquidityBitmap.unsetBitsRange(255,400)
                expect(await liquidityBitmap.hasLiquidity(255)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(400)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(401)).eq(true)
            })

            it("should unset bit from 600 to 255", async () => {
                await setBitsInRage2(255, 600);
                const result = await liquidityBitmap.unsetBitsRange(600, 255)
                expect(await liquidityBitmap.hasLiquidity(255)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(600)).eq(false)
            })

            it("should unset bit from 400 to 255, should not unset 401 and 254", async () => {
                await setBitsInRage2(254, 401);
                const resul = await liquidityBitmap.unsetBitsRange(400,255)
                expect(await liquidityBitmap.hasLiquidity(255)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(400)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(401)).eq(true)
                expect(await liquidityBitmap.hasLiquidity(254)).eq(true)
            })

            it("should unset bit from 256 to 400, should not unset 255 and 401", async () => {
                await setBitsInRage2(255, 401);
                const resul = await liquidityBitmap.unsetBitsRange(256,400)
                expect(await liquidityBitmap.hasLiquidity(255)).eq(true)
                expect(await liquidityBitmap.hasLiquidity(256)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(400)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(401)).eq(true)
            })

            it("should unset bit from 100 to 213, should not unset 214", async () => {
                await setBitsInRage2(100, 214);
                const result = await liquidityBitmap.unsetBitsRange(100, 213)
                expect(await liquidityBitmap.hasLiquidity(100)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(213)).eq(false)
                expect(await liquidityBitmap.hasLiquidity(214)).eq(true)
            })
        })
    });

...
import {
    ...
    FundingRateTest, UserGateway, LiquidatorGateway, UserGatewayTest
} from "../../typeChain";
import {
    ClaimFund,
    LimitOrderReturns,
...
describe("PositionHouse_02", () => {
    ...
    describe('debug DTP', async function () {
        it('should got list order pending', async function () {
            ...
            numberOfOrder++
            const listOrderPending = await userGateway.getListOrderPending(positionManager.address, trader0.address)
        ...
        it('get claim amount of position created by limit order and closed by limit order', async function () {
            ...
            const claimableAmount = await userGateway.getClaimAmount(positionManager.address, trader0.address)
        ...
        it('get claimable amount correct with limit order and cancelled limit order', async function () {
            ...
            // console.log(await userGateway.connect(trader0).getPosition(positionManager.address, trader0.address))
        ...
        it('open limit order at current price', async function () {
            ...
            // console.log((await userGateway.getRemovableMargin(positionManager.address, trader0.address)).toString())

            // await userGateway.connect(trader0).removeMargin(positionManager.address, 500000)
        ...
        it('get claim amount of partial filled', async function () {
            ...
            console.log((await userGateway.getPosition(positionManager.address, trader0.address)).toString())
            const claimableAmount = await userGateway.getClaimAmount(positionManager.address, trader0.address)
        ...
        it('close order in current price and toggle current price', async function () {
            ...
            //
            // console.log((await userGateway.getPosition(positionManager.address, trader0.address)).toString())
            // console.log((await userGateway.getClaimAmount(positionManager.address, trader0.address)).toString())
        })
        ...
        it('has market transaction in limit order in current price', async function () {
            ...
            console.log(4318)
            const quantityTrader0 = (await userGateway.getPosition(positionManager.address, trader0.address)).quantity.toString()
            const quantityTrader1 = (await userGateway.getPosition(positionManager.address, trader1.address)).quantity.toString()
            console.log(await userGateway.getListOrderPending(positionManager.address, trader1.address))
        ...
        //     })
        //
        //     await userGateway.connect(trader1).liquidate(positionManager.address, trader0.address)
        // })
        ...
        it("cancel close position limit order 100% then open new limit order", async function () {
            ...
            await userGateway.connect(trader0).closeLimitPosition(positionManager.address, 499000, 10)
        ...
        it("open order with same price and different side", async function () {
            ...
            const getTrader0Position = await userGateway.getPosition(positionManager.address, trader0.address)
            console.log(getTrader0Position)
            expect(getTrader0Position.quantity).eq(-10)

            const getTrader1Position = await userGateway.getPosition(positionManager.address, trader1.address)
        ...
        //     );
        //
        //     let claimableAmountTrader0 = await userGateway.getClaimAmount(positionManager.address, trader0.address)
        ...
        //     })
        //
        //     claimableAmountTrader0 = await userGateway.getClaimAmount(positionManager.address, trader0.address)
        //     console.log("1    ", claimableAmountTrader0.toString())
        //
        //     await expect(userGateway.connect(trader0).closeLimitPosition(positionManager.address, 510000, 10)).to.be.revertedWith('23')
        //
        //     claimableAmountTrader0 = await userGateway.getClaimAmount(positionManager.address, trader0.address)
        ...
        //     );
        //
        //     const listOrderPendingTrader0 = await userGateway.getListOrderPending(positionManager.address, trader0.address)
        ...
        it("should cancel close limit order", async function () {
            ...
            await userGateway.connect(trader0).closeLimitPosition(positionManager.address, 500000, 7);
            await userGateway.connect(trader0).closeLimitPosition(positionManager.address, 500000, 2);
            console.log(await userGateway.getListOrderPending(positionManager.address, trader0.address))
        ...
        it("should close position while still have pending order", async function () {
            ...
            await expect(userGateway.connect(trader0).closePosition(positionManager.address, 18)).to.be.revertedWith("22")
            return;



            console.log(await userGateway.getPosition(positionManager.address, trader0.address))
        ...
        it("should get correct list pending order", async function () {
            ...
            await expect(userGateway.connect(trader0).closeLimitPosition(positionManager.address, 510000, BigNumber.from('18000000000000000000'))).to.be.revertedWith("22")
            return;
        ...
        it("should get correct quantity of position when partial filled", async function (){
            ...
            console.log((await userGateway.getPosition(positionManager.address, trader0.address)).quantity.toString())
            ...
            console.log((await userGateway.getListOrderPending(positionManager.address, trader0.address))[0])
        ...
        it("test get position after closed", async function (){
            ...
            const trader0PositionData = await userGateway.getPosition(positionManager.address, trader0.address)
        ...
        it("test partial fill after close a limit order", async () => {
            ...
            const listOrderPending1 = await userGateway.getListOrderPending(positionManager.address, trader0.address)
            ...
            await userGateway.connect(trader0).cancelLimitOrder(positionManager.address, 0, 0)
            ...
            const listOrderPending2 = await userGateway.getListOrderPending(positionManager.address, trader0.address)
        ...
        it("should change nothing when cancel a fully filled order", async () => {
            ...
            const position = await userGateway.getPosition(positionManager.address, trader0.address)
            expect(position.quantity.toString()).eq("-7")
            await expect(userGateway.cancelLimitOrder(positionManager.address, 1, 0)).to.be.revertedWith("21")
            expect(position.quantity.toString()).eq("-7")
        })
        ...
        it("should not open new limit order when internal close in openLimitOrder at the same pip", async () => {
            ...
            const positionDataOfTrader0 = await userGateway.getPosition(positionManager.address, trader0.address)
            const pendingOrdersOfTrader0 = await userGateway.getPosition(positionManager.address, trader0.address)
        ...
        it("should stay remain quantity when cancel a reverse limit order", async () => {
            ...
            const positionDataBeforeCancel = (await userGateway.getPosition(positionManager.address, trader1.address)).toString()
            ...
            const positionDataAfterCancel = (await userGateway.getPosition(positionManager.address, trader1.address)).toString()
        ...
        it("should partially fill limit order correctly", async () => {
            ...
            await userGateway.connect(trader1).cancelLimitOrder(positionManager.address, 1, 0)
            ...
            const trader1PendingOrder = await userGateway.getListOrderPending(positionManager.address, trader1.address)
            ...
            await userGateway.connect(trader2).closeLimitPosition(positionManager.address, 490000, BigNumber.from('5'))

            await userGateway.connect(trader1).closeLimitPosition(positionManager.address, 490000, BigNumber.from('6'))
            const trader1PendingOrderAfterClose = await userGateway.getListOrderPending(positionManager.address, trader1.address)
        ...
        //     await userGateway.connect(trader1).closePosition(positionManager.address, BigNumber.from("4"))
        ...
        //         _trader: trader4
        //     })
        //     const trader1CanClaimAmount = (await userGateway.getClaimAmount(positionManager.address, trader1.address)).toString()
        ...
        it("should claim fund success after close by limit order 2", async () => {
            ...
            const balanceBeforeCloseMarket = await bep20Mintable.balanceOf(trader1.address)
            await userGateway.connect(trader1).closePosition(positionManager.address, BigNumber.from("10"))
            const balanceAfterCloseMarket = await bep20Mintable.balanceOf(trader1.address)
            const paybackQuantity = BigNumber.from(balanceAfterCloseMarket).sub(BigNumber.from(balanceBeforeCloseMarket))
        ...
        it("should return margin when close position correct", async () => {
            ...
            await userGateway.connect(trader1).addMargin(positionManager.address, BigNumber.from('3500'))

            console.log("before first time reverse")
            let balanceBeforeReversePosition = await bep20Mintable.balanceOf(trader1.address)
            await userGateway.connect(trader1).closePosition(positionManager.address, BigNumber.from('7'))
            ...
            await expect((await userGateway.getPosition(positionManager.address, trader1.address)).margin).eq("1061")
            console.log("before second time reverse")
            balanceBeforeReversePosition = await bep20Mintable.balanceOf(trader1.address)
            await userGateway.connect(trader1).closePosition(positionManager.address, BigNumber.from('2'))
            ...
            await expect((await userGateway.getPosition(positionManager.address, trader1.address)).margin).eq("354")
        })

        it("should cannot liquidate user don't have position", async () => {
            const maintenanceDetail = await userGateway.getMaintenanceDetail(positionManager.address, trader1.address, 1)
        ...
        it("should liquidate success", async () => {
            ...
            await liquidatorGateway.liquidate(positionManager.address, trader1.address)
        })
        ...
        it("should get back correct amount after close limit position by limit and market order", async () => {
            ...
            console.log("getClaimableAmount", (await userGateway.getClaimAmount(positionManager.address, trader1.address)).toString())
        ...
        it("should cancel pending order success of liquidated user", async () => {
            ...
            await userGateway.connect(trader1).cancelLimitOrder(fundingRateTest.address, 0, 0)
        })
        ...
        it("pending order should be filled normally after position got liquidated", async () => {
            ...
            console.log("get list order pending ##################", (await userGateway.getListOrderPending(fundingRateTest.address, trader1.address)))
        ...
        it("should increase claimable fund when partial filled an increase limit order", async () => {
            ...
            const claimableAmountAfterFirstOrder = (await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)).toString()
            ...
            const claimableAmountAfterSecondOrder = (await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)).toString()
    ...
    describe("should calculate pnl correctly when close position by market order", async () => {
        it("should get pnl correctly when close position after increased", async () => {
            await openLimitPositionAndExpect({
                limitPrice: 4800,
                side: SIDE.LONG,
                leverage: 10,
            ...
            await expect(balanceAfterClose.sub(balanceBeforeClose)).eq("8150")
        })

        it("should get pnl correctly when close position after added margin", async () => {
            await openLimitPositionAndExpect({
                limitPrice: 4900,
                side: SIDE.LONG,
                leverage: 10,
            ...
            await expect(balanceAfterClose.sub(balanceBeforeClose)).eq('5400')
        })

        it("reduce position then close by limit", async () => {
            await openLimitPositionAndExpect({
                limitPrice: 5000,
                side: SIDE.SHORT,
                leverage: 10,
            ...
            await expect(balanceAfterClose.sub(balanceBeforeClose)).eq('1932')
            ...
            const claimableAmountAfterCloseLimit = await userGateway.getClaimAmount(positionManager.address, trader1.address)
            await expect(claimableAmountAfterCloseLimit.toString()).eq('2898')
        })

        it("close position by a limit order in reverse price", async () => {
            await openLimitPositionAndExpect({
                limitPrice: 4900,
                side: SIDE.LONG,
                leverage: 10,
            ...
            const balanceAfterClose = await bep20Mintable.balanceOf(trader1.address)
            await expect(balanceAfterClose.sub(balanceBeforeClose)).eq('3090')
            ...
            const claimableAmountAfterCloseLimit = await userGateway.getClaimAmount(positionManager.address, trader1.address)
            await expect(claimableAmountAfterCloseLimit.toString()).eq('1920')
        })

        it("reduce position by limit then close by market", async () => {
            await openLimitPositionAndExpect({
                limitPrice: 5100,
                side: SIDE.SHORT,
                leverage: 10,
            ...
            const claimableAmountAfterCloseLimit = await userGateway.getClaimAmount(positionManager.address, trader1.address)
            await expect(claimableAmountAfterCloseLimit.toString()).eq('6100')
        ...
        it("should reverse position correctly", async () => {
            await changePrice({
                limitPrice: 2750,
                toHigherPrice: false
            })
            // Step 1: trader1 open limit order limitPrice = 2750, quantity = 5, leverage = 5
            await openLimitPositionAndExpect({
                limitPrice: 2750,
                side: SIDE.SHORT,
                leverage: 5,
            ...
            // expect position data of trader 1 after step 1
            await expectMarginPnlAndOP({
                positionManagerAddress: positionManager.address,
                traderAddress: trader1.address,
                expectedOpenNotional: toWei(13750),
                expectedMargin: toWei(2750),
                expectedPnl: 0,
                expectedQuantity: toWei(-5),
            });

            // Step 2: trader1 open market order limitPrice quantity = 5, leverage = 10
            await openMarketPosition({
                    quantity: BigNumber.from(toWei('5')),
                    leverage: 10,
                    side: SIDE.SHORT,
            ...
            // expect position data of trader 1 after step 2
            await expectMarginPnlAndOP({
                positionManagerAddress: positionManager.address,
                traderAddress: trader1.address,
                expectedOpenNotional: toWei(27500),
                expectedMargin: toWei(4125),
                expectedPnl: 0,
                expectedQuantity: toWei(-10),
            });

            // Step 3: trader1 open limit order short limitPrice = 2800, quantity = 5, leverage = 10
            await openLimitPositionAndExpect({
                limitPrice: 2800,
                side: SIDE.SHORT,
                leverage: 10,
            ...
            await expectMarginPnlAndOP({
                positionManagerAddress: positionManager.address,
                traderAddress: trader1.address,
                expectedOpenNotional: toWei(41500),
                expectedMargin: toWei(5525),
                expectedPnl: toWei(-500),
                expectedQuantity: toWei(-15),
            });
            ...
            console.log((await userGateway.getListOrderPending(positionManager.address, trader1.address)).toString())

            await userGateway.connect(trader1).cancelLimitOrder(positionManager.address, 2, 0)

            // Step 4: trader1 open limit order long limitPrice = 2900, quantity = 5, leverage = 10, filled 4
            await openLimitPositionAndExpect({
                limitPrice: 2900,
                side: SIDE.LONG,
                leverage: 10,
            ...
            await expectMarginPnlAndOP({
                positionManagerAddress: positionManager.address,
                traderAddress: trader1.address,
                expectedOpenNotional: '30433333600000000000000',
                expectedMargin: '4051666702168674698796',
                expectedPnl: '-1466666400000000000000',
                expectedQuantity: toWei(-11),
            });
    ...
    describe('liquidate and orderbook', async () => {
        // it('should liquidate success and get correct order book', async () => {
        //     await openLimitPositionAndExpect({
        //         limitPrice: 5100,
        //         side: SIDE.SHORT,
        //         leverage: 100,
        ...
        //     })
        //
        //     await userGateway.connect(trader3).openMarketPositionByQuote(fundingRateTest.address, 0, toWei('55000'), 1)
        //
        //     await fundingRateTest.setMockPrice(5500, 5500)
        //
        //     await liquidatorGateway.liquidate(fundingRateTest.address, trader1.address)
        //
        //     console.log((await userGateway.getOrderbook(fundingRateTest.address, 3)).toString())
        //
        //     console.log((await fundingRateTest.getCurrentSingleSlot()).toString())
        // })
    })
})

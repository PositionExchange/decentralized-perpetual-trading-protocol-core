...
describe('Position Manager', async function () {
    ...
    beforeEach(async () => {
        ...
        accessController = (await accessControllerFactory.deploy()) as unknown as AccessController
        ...
        await positionManager.initialize(BigNumber.from(200), bep20Mintable.address, ethers.utils.formatBytes32String('BTC'), BigNumber.from(100), BigNumber.from(10000), BigNumber.from(10000), BigNumber.from(3000), BigNumber.from(1000), '0x5741306c21795FdCBb9b265Ea0255F499DFe515C'.toLowerCase());
    ...
    async function createLimitOrder(trader: string, pip: number, size: number | string, isBuy: boolean) {
        return positionManager.openLimitPosition(trader, pip, size, isBuy);
    }
    ...
    async function createLimitOrderAndVerify(pip: number, size: number | string, isBuy: boolean) {
        ...
        await createLimitOrder('0x0000000000000000000000000000000000000000', pip, size, isBuy);
    ...
    async function marketBuy(trader:string, size: number | string, isBuy: boolean = true) {
        return positionManager.openMarketPosition(trader, size, isBuy)
    }
    ...
    const shouldOpenLimitAndVerify = async function (pip: number, size: number, expectOut: number, isBuy: boolean = true) {
        const [caller] = await ethers.getSigners()
        await accessController.updateValidatedContractStatus(caller.address, true)
        const tx = await positionManager.openLimitPosition('0x0000000000000000000000000000000000000000', pip, size, isBuy)
        const receipt = await ethers.provider.getTransactionReceipt(tx.hash)

        // const interfaceLimitOrderCreatedEvent = new ethers.utils.Interface(["event LimitOrderCreated(uint64 orderId, address trader, uint128 pip, bool isBuy, uint128 size)"])
        // const limitOrderCreatedEvent = interfaceLimitOrderCreatedEvent.decodeEventLog("LimitOrderCreated", receipt.logs[3].data, receipt.logs[3].topics)
        // expect(limitOrderCreatedEvent.trader).to.equal('0x0000000000000000000000000000000000000000')
        // expect(limitOrderCreatedEvent.pip).to.equal(pip)
        // expect(limitOrderCreatedEvent.isBuy).to.equal(isBuy)
    ...
    describe('buy sell in multiple ranges', async function () {
        ...
        it('market short should revert transaction due to price too far from index price', async function () {
            ...
            await expect(marketBuy('0x0000000000000000000000000000000000000000', 10, false)).to.be.revertedWith("VM Exception while processing transaction: reverted with reason string '25'")
        });
        it('market long should revert transaction due to price too far from index price', async function () {
            ...
            await expect(marketBuy('0x0000000000000000000000000000000000000000', 20, true)).to.be.revertedWith("VM Exception while processing transaction: reverted with reason string '25'")
        });
    });
    ...
    describe("debug finding pip buy error", async () => {
        it('should debug case 1', async  function () {
            ...
            await createLimitOrderInPipRanges(pips, pipSizes, false)
            await marketBuy('0x0000000000000000000000000000000000000000', '17900000000000001', true)
        });
    })

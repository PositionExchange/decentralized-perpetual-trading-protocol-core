...
import {
    ...
    BEP20Mintable,
    InsuranceFund,
    UserGateway, PositionHouseConfigurationProxy, LiquidatorGateway, FundingRateTest, UserGatewayTest
} from "../../typeChain";
import {
...
describe("PositionHouse_01", () => {
    ...
    const openMarketPosition = async ({
        ...
        const task = userGateway.connect(instanceTrader).openMarketPosition(
        ...
        const positionInfo = await userGateway.getPosition(_positionManager.address, trader) as unknown as PositionData;
    ...
    async function getOrderIdByTx(tx: any) {
        ...
        const orderId = ((receipt?.events || [])[0]?.args || [])['orderId'] || ((receipt?.events || [])[1]?.args || [])['orderId'] || ((receipt?.events || [])[2]?.args || [])['orderId'] || ((receipt?.events || [])[3]?.args || [])['orderId'] || ((receipt?.events || [])[4]?.args || [])['orderId'] || ((receipt?.events || [])[5]?.args || [])['orderId']
        return orderId
    }
    ...
    async function openLimitPositionAndExpect({
        ...
        const tx = await userGateway.connect(_trader).openLimitOrder(_positionManager.address, side, quantity, priceToPip(Number(limitPrice)), leverage)
    ...
    async function getPositionNotionalAndUnrealizedPnl(positionManagerAddress: string, traderAddress: string): Promise<NotionalAndUnrealizedPnlReturns> {
        ...
        return userGateway.getPositionNotionalAndUnrealizedPnl(positionManagerAddress, traderAddress, BigNumber.from(1), oldPosition)
    ...
    describe('openMarketPosition', async () => {
        ...
        describe('close and open reverse', function () {
            ...
            //     const positionData = (await userGateway.getPosition(positionManager.address, trader.address)) as unknown as PositionData
    ...
    describe('openLimitPosition', async () => {
        ...
        it('should open limit a position', async function () {
            ...
            // const pendingOrder = await userGateway["getPendingOrder(address,bytes)"](positionManager.address, orderId)
            // const pendingOrder = await userGateway.getPendingOrder(positionManager.address, pip, orderId);
            //
            // expect(pendingOrder.isFilled).eq(true)


            const positionData = await userGateway.getPosition(positionManager.address, trader.address)
        ...
        describe('it will error', async () => {
            it('refill the full filled order', async () => {
                ...
                // const pendingOrder2 = await userGateway.getPendingOrder(positionManager.address, response2.pip, response2.orderId);
                ...
                // const pendingOrder3 = await userGateway.getPendingOrder(positionManager.address, response3.pip, response3.orderId);
                // expect(pendingOrder3.isFilled).eq(false)
                // expect(pendingOrder3.size).eq(100);
                // expect(pendingOrder3.partialFilled).eq(60);
                const positionData1 = await userGateway.getPosition(positionManager.address, trader.address)
                const positionDataTrader2 = await userGateway.getPosition(positionManager.address, trader2.address)
        ...
        describe('should open and cancel', async () => {
            ...
            it('cancel with three order with the same price, one filled one cancel one partial filled', async () => {
                ...
                const positionData2 = await userGateway.getPosition(positionManager.address, trader.address)
                // margin = quantity * price / leverage = 4990 * 100 / 10
                // NEED UPDATE can't get margin, need leverage in limit order to calculate margin
                // expect(positionData.margin.toNumber()).eq(4990 * 100 / 10)
            ...
            it('open 3 limit order with pending all, and cancel #2', async () => {
                ...
                console.log("response 2 order id", response2.orderId)
                let response3 = (await openLimitPositionAndExpect({
                    limitPrice: 4988,
                    side: SIDE.LONG,
                    leverage: 10,
                ...
                await cancelLimitOrder(positionManager.address, trader, '1', response2.pip.toString());

                // const pendingOrder1 = await userGateway.getPendingOrder(positionManager.address, response1.pip, response1.orderId);
            ...
            it('open 3 limit order, open market and cancel #1 ', async () => {
                ...
                console.log("response 1", response1.orderId)
                let response2 = (await openLimitPositionAndExpect({
                    limitPrice: 5008,
                    side: SIDE.SHORT,
                    leverage: 10,
                ...
                // cancel order #1
                await cancelLimitOrder(positionManager.address, trader, '1', response1.pip.toString());
        ...
        describe('should close position with close limit', async () => {
            ...
            describe('should close position with close limit 100%', async () => {
                ...
                //     return;
                //
                //     // const dataClaim = (await userGateway.canClaimFund(positionManager.address, trader.address)) as unknown as ClaimFund;
            ...
            describe('close position with close limit when has openMarketPosition', async () => {
                it('close limit when has openMarketPosition SHORT and have no partialFilled before', async () => {
                    ...
                    const positionDataTrader1 = (await userGateway.getPosition(positionManager.address, trader1.address)) as unknown as PositionData;
                ...
                it('ERROR open reverse: close limit when has openMarketPosition SHORT and has partialFilled before 02', async () => {
                    ...
                    const positionDataTrader1 = (await userGateway.getPosition(positionManager.address, trader3.address)) as unknown as PositionData;
        ...
        describe('should reduce open limit', async () => {
            ...
            it('ERROR self filled market: open limit order has been partialFilled and open market with reduce position', async () => {
                ...
                // const pendingOrder = await userGateway.getPendingOrder(positionManager.address, response1.pip, response1.orderId);
        ...
        describe('liquidate with open limit order', async () => {
            ...
            //
            //
            //     const maintenanceDetail = (await userGateway.getMaintenanceDetail(positionManager.address, trader.address)) as unknown as MaintenanceDetail;
            //     const positionDataTrader0 = (await userGateway.getPosition(positionManager.address, trader.address)) as unknown as PositionData;
            ...
            //     })
            //
            //     await userGateway.liquidate(positionManager.address, trader.address);
            //
            //     const positionNotionalAndPnLTrader0AfterLiquidate = await getPositionNotionalAndUnrealizedPnl(
            //         positionManager.address,
            //         trader.address
            //     )
            //     const maintenanceDetailTrader0AfterLiquidate = (await userGateway.getMaintenanceDetail(positionManager.address, trader.address)) as unknown as MaintenanceDetail;
            //
            //     const positionDataTrader0AfterLiquidate = (await userGateway.getPosition(positionManager.address, trader.address)) as unknown as PositionData;
    ...
    describe('adjust margin', async function () {
        it('add margin', async function () {
            ...
            const positionData = (await userGateway.getPosition(positionManager.address, trader1.address)) as unknown as PositionData;
            console.log('positionData margin: ', positionData.margin.toString());
            expect(positionData.margin).eq(1000)


            await userGateway.connect(trader1).addMargin(positionManager.address, BigNumber.from('100'))

            const positionData1 = (await userGateway.getPosition(positionManager.address, trader1.address)) as unknown as PositionData;
        ...
        //
        //
        //     await userGateway.connect(trader1).removeMargin(positionManager.address, BigNumber.from('0'))
        //
        //     const positionData1 = (await userGateway.getPosition(positionManager.address, trader1.address)) as unknown as PositionData;
        //
        //     console.log('positionData after add margin margin: ', positionData1.margin.toString());
    ...
    describe('liquidate position', async function () {
        ...
        //
        //
        //     const positionData1 = (await userGateway.getPosition(positionManager.address, trader.address)) as unknown as PositionData;
        //

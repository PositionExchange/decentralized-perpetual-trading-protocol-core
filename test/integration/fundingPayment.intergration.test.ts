...
import {
    ...
    UserGatewayTest,
    PositionManager, LiquidatorGateway
} from "../../typeChain";
...
describe('Test Funding Payment Intergration', function () {
    ...
    describe('funding rate with limit order', function () {
        ...
        it("EGE_TC_62: should calculate correct funding payment of position created by market order", async () => {
            ...
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 1", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            ...
            const claimableFundAfterStep1 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            const claimableFundAfterStep2 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 2", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            ...
            const claimableFundAfterStep4 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 3", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())

            // claimbleAmount = claimableAmount4 + fundingPayment = 2445.79037864 + 1.0651949572767516 = 2446.855573597277
            const claimableFundAfterStep5 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            const claimableFundAfterStep6 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 4", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())

            // claimbleAmount = claimableAmount6 + fundingPayment = 7296.851919412723 - 3.105077915773646 = 7293.746841496949
            const claimableFundAfterStep7 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            await userGateway.connect(trader2).claimFund(fundingRateTest.address)
        ...
        it("EGE_TC_63: should calculate correct funding payment of position created by market order 2", async () => {
            ...
            const claimableFundAfterStep3 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
        ...
        it("EGE_TC_64: should calculate correct funding payment of position created by limit order", async () => {
            ...
            console.log("get position data in step 4", (await userGateway.getPosition(fundingRateTest.address, trader1.address)).toString())
            const claimableFundAfterStep4 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            ...
            const claimableFundAfterStep8 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
        ...
        it("EGE_TC_65", async () => {
            ...
            console.log((await userGateway.getPosition(fundingRateTest.address, trader1.address)).margin.toString())
            const claimableFundAfterStep6 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
        ...
        it("EGE_TC_71", async () => {
            ...
            const claimableFundAfterStep6 = await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)
            expect(claimableFundAfterStep6.toString()).eq("293159376922666666667")
    ...
    describe("calculate funding payment without manual margin", async () => {
        ...
        it("EGE-241: should calculate funding payment based on position margin without manual margin", async () => {
            ...
            const premiumFraction1 = (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString()
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 1", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            ...
            const premiumFraction2 = (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString()
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 2", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            // funding payment of LONG position = (premiumFraction2 - premiumFraction1) * -margin / 10**10 = (-24509803 + 8169934) * -4900 / 10**10 = 8.00653581
            const fundingPaymentStep2 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            await expect(fundingPaymentStep2).eq(toWei('8.00653581'))

            // Step 3
            console.log("STEP 3")
            await userGateway.connect(trader1).addMargin(fundingRateTest.address, toWei('200'))
            const fundingPaymentStep3 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            ...
            const marginInStep4 = (await userGateway.getPosition(fundingRateTest.address, trader1.address)).margin.toString()
            ...
            const fundingPaymentStep5 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            await expect(fundingPaymentStep5).eq(toWei('-25.270016339525'))

            // Step 6
            console.log("STEP 6")
            await userGateway.connect(trader1).removeMargin(fundingRateTest.address, toWei('100'))
            // trader1 margin = oldMargin + fundingPaymentStep2 + manualMargin + newMargin = 10308.00653581 - 100 = 10208.00653581
            const marginInStep6 = (await userGateway.getPosition(fundingRateTest.address, trader1.address)).margin.toString()
            await expect(marginInStep6).eq(toWei('10208.00653581'))

            const fundingPaymentStep6 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            // same as step 5
            ...
            const fundingPaymentStep7 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            ...
            const marginInStep8 = (await userGateway.getPosition(fundingRateTest.address, trader1.address)).margin.toString()
            ...
            const claimableAmount = (await userGateway.getClaimAmount(fundingRateTest.address, trader1.address)).toString()
        ...
        it("EGE-242: should calculate funding payment based on position margin without manual margin", async () => {
            ...
            const premiumFraction4 = (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString()
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 4", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            // funding payment of SHORT position = (premiumFraction4 - premiumFraction1) * margin / 10**10 = (5024511 + 8169934) * 4900 / 10**10 = 8.50694439
            const fundingPaymentStep4 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
            ...
            await fundingRateTest.setMockPrice(BigNumber.from("5100"), BigNumber.from("4900"))
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 7", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
        ...
        it("EGE-244: should calculate funding payment based on position margin without manual margin 2", async () => {
            ...
            await userGateway.payFunding(fundingRateTest.address)
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 6", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            ...
            const premiumFraction8 = (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString()
            console.log("LATEST CUMULATIVE PREMIUM FRACTION 8", (await userGateway.getLatestCumulativePremiumFraction(fundingRateTest.address)).toString())
            ...
            const fundingPaymentStep8 = (await userGateway.getFundingPaymentAmount(fundingRateTest.address, trader1.address)).toString()
    ...
    describe("get maintenance detail with funding payment", async () => {
        ...
        it("should get maintenance margin success", async () => {
            ...
            console.log((await userGateway.getMaintenanceDetail(fundingRateTest.address, trader1.address, 0)).toString())
